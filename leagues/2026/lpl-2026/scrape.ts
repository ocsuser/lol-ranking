/**
 * Scrape LPL 2026 depuis gol.gg
 * Trois splits : Split 1 + Split 1 Playoffs + Split 2
 * Calcule le LIR pour chaque split + les trois réunis
 * Export → frontend/public/leagues/lpl-2026/export.json
 *
 * Usage : npx tsx leagues/lpl-2026/scrape.ts
 */
...
const SPLITS = [
  { key: 'split1',          name: 'LPL 2026 Split 1',          season: 'ALL' },
  { key: 'split1playoffs',  name: 'LPL 2026 Split 1 Playoffs',  season: 'ALL' },
  { key: 'split2',          name: 'LPL 2026 Split 2',          season: 'ALL' },
];
const COMBINED_NAME = 'LPL 2026';
...
const split1Players = players.filter(p => p.rows.split1);
const split1Results = computeRatingsForGroup(split1Players, p => p.rows.split1);
split1Players.forEach((p, i) => { p.lirSplit1 = split1Results[i]; });

const split1poPlayers = players.filter(p => p.rows.split1playoffs);
const split1poResults = computeRatingsForGroup(split1poPlayers, p => p.rows.split1playoffs);
split1poPlayers.forEach((p, i) => { p.lirSplit1Playoffs = split1poResults[i]; });

// Split 1 Combined (Split 1 + Playoffs)
const split1CombPlayers = players.filter(p => p.rows.split1 || p.rows.split1playoffs);
split1CombPlayers.forEach(p => {
  const rows = [p.rows.split1, p.rows.split1playoffs].filter(Boolean);
  ...
  comb.pentaKills = rows.reduce((s: number, r: any) => s + r.pentaKills, 0);
  comb.soloKills  = rows.reduce((s: number, r: any) => s + r.soloKills,  0);
  p.split1Combined = comb;
});
const split1CombResults = computeRatingsForGroup(split1CombPlayers, p => p.split1Combined);
split1CombPlayers.forEach((p, i) => { p.lirSplit1Combined = split1CombResults[i]; });

const split2Players = players.filter(p => p.rows.split2);
const split2Results = computeRatingsForGroup(split2Players, p => p.rows.split2);
split2Players.forEach((p, i) => { p.lirSplit2 = split2Results[i]; });
...
const exportData = {
  metadata: {
    ...
    tournaments: [{ name: COMBINED_NAME, league: 'LPL', year: 2026, split: 'Full Year', scrapedAt: new Date().toISOString() }],
  },
  ...
  players: players.map(p => {
    ...
    if (p.rows.split1) {
      tournaments[SPLITS[0].name] = { ...toStats(p.rows.split1), ...(p.lirSplit1 ?? {}) };
    }
    if (p.rows.split1playoffs) {
      tournaments[SPLITS[1].name] = { ...toStats(p.rows.split1playoffs), ...(p.lirSplit1Playoffs ?? {}) };
    }
    if (p.split1Combined) {
      tournaments['LPL 2026 Split 1 Combined'] = { ...toStats(p.split1Combined), ...(p.lirSplit1Combined ?? {}) };
    }
    if (p.rows.split2) {
      tournaments[SPLITS[2].name] = { ...toStats(p.rows.split2), ...(p.lirSplit2 ?? {}) };
    }
