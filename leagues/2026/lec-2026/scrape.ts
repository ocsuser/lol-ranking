 ...
 * Export → leagues/lec-2026/export.json
 */
...
const SPLITS = [
  ...
  { key: 'spring',   name: 'LEC 2026 Spring Season',   season: 'ALL' },
];
...
// Versus Combined (Season + Playoffs uniquement, sans Spring)
const versusPlayers = players.filter(p => p.rows.season || p.rows.playoffs);
versusPlayers.forEach(p => {
  const versusRows = [p.rows.season, p.rows.playoffs].filter(Boolean);
  const totalGames = versusRows.reduce((s: number, r: any) => s + r.games, 0);
  const versusCombined: any = { games: totalGames };
  for (const stat of AVG_STATS) versusCombined[stat] = weightedAvg(versusRows, stat);
  versusCombined.pentaKills = versusRows.reduce((s: number, r: any) => s + r.pentaKills, 0);
  versusCombined.soloKills  = versusRows.reduce((s: number, r: any) => s + r.soloKills, 0);
  p.versusCombined = versusCombined;
});
const versusResults = computeRatingsForGroup(versusPlayers, p => p.versusCombined);
versusPlayers.forEach((p, i) => { p.lirVersus = versusResults[i]; });

// Combined global (tous les splits)
...
const exportData = {
  ...
  players: players.map(p => {
    ...
    if (p.rows.spring) {
      tournaments[SPLITS[2].name] = {
        ...toStats(p.rows.spring),
        ...(p.lirSpring ?? {}),
      };
    }
    if (p.versusCombined) {
      tournaments['LEC 2026 Versus'] = {
        ...toStats(p.versusCombined),
        ...(p.lirVersus ?? {}),
      };
    }
...
const outPath = path.join(__dirname, '../../../frontend/public/leagues/2026/lec-2026/export.json');
